...
On macOS, the script briefly opens a native window to obtain a graphics context;
the Open3D 0.19 macOS wheel does not support EGL offscreen rendering. Other
platforms use `OffscreenRenderer` and need a working graphics driver.

This example uses Open3D's modern Filament scene API, with matte pastel
materials, a grey floor curved into a backdrop, directional shadows, ambient
occlusion, environment illumination, and a local fill light. Edit `PALETTE`,
`material`, and the lighting settings in `build_scene` to adjust the appearance.
...
artifacts still possible. It does not reproduce Cycles' path-traced area lights.
...
cross-section lofting helpers. The existing interactive renderer is unchanged.
...
This uses the existing `ViserRenderer.show()` geometry pipeline, the same five
...
Open3D's ambient occlusion or material response exactly. The current renderer
keeps capped link boundaries, which remain visible near the tentacle tips.
