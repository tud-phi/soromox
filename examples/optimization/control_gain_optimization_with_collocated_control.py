...
reference_trajectory = ReferenceTrajectory(ts=save_ts, x_des_fn=x_des_fn)
...
# Run Simulation
# =========================================================================
# Time-step for storing the result
save_dt = solver_dt
...
# Box Constraints: mask gradients of variables that exceeds threshold
threshold_box_maps = {
    "P_gain": jnp.array([0.0, 100.0]),
    "I_gain": jnp.array([0.0, 50.0]),
    "D_gain": jnp.array([0.0, 20.0]),
}

# Gradient function
gradient_fn = jax.jit(jax.value_and_grad(evaluate_closed_loop_system, 0, has_aux=True))
...
while i < num_iters:
    ...
    # Store data
    time_iter.append(step_duration)
    loss_tot.append(loss)
    t_ts_tot.append(aux["t_ts"])
    q_ts_tot.append(aux["q_ts"])
    qd_ts_tot.append(aux["qd_ts"])
    ...
    print(
        "Completion: {:3.1f} %  |  iteration {:>4d} of {:<4d}  |  iter time = {:>.2f} s  |  ETA = {}/{}/{} {:>2d}:{:>2d}".format(
            100 * (i + 1) / num_iters,
            i + 1,
            num_iters,
            step_duration,
...
# Log
print(
    "Completion: {:3.1f} %  |  iteration {:>4d} of {:<4d}  |  iter time = {:>.4f} s".format(
        100 * i / num_iters, i, num_iters, step_duration
    )
)
