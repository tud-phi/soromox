"""Simulate and render the first-joint-actuated Soft Pendubot."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--viser-port", type=int, default=8083)
    return parser.parse_args()


def main() -> None:
    args = parse_args()
    robot = make_soft_pendubot(SoftSwingUpPendulumConfig())
    q0 = pendubot_upright_configuration().at[0].add(args.initial_angle_offset)
    ...
    figure_path = args.figures_dir / "simulate_soft_pendubot.png"
    save_summary_figure(
        robot,
        trajectory,
        figure_path,
        coordinate_labels=("theta1", "a10", "a11", "theta2", "a20", "a21"),
        show=not args.no_show,
    )
    print(f"Summary figure saved to {figure_path}")
    if not args.no_viser:
        record_path = (
            args.videos_dir / "simulate_soft_pendubot_viser.mp4"
