"""AM-I Support: soromox rollout of the tip162219 PCS fit (T162219_pcs).

Sibling of `simulate_isupport_exp_last.py`, restricted to ONE run: the
`T162219_pcs` MATLAB fit -- a new bag family (20260724 dataset),
`Eco50_2.5bar_default_tip_162219.bag`, bang-bang square wave driving both
modules IN PHASE (unlike exp_veronica's single-channel step). perm [2 1 3] /
roll +8 deg / clock 0, 430 mm geometry, Eta + 6 INDEPENDENT per-chamber gains
(not tied -- see TIP162219_IDENTIFICATION_REPORT.md: two of module 1's three
gains sit near the 0.1 lower bound, traced to module1/module2 being driven
identically, a data-imposed identifiability limit, not a wiring bug). Every
parameter is read from the `.mat` exported by `python_exporter_tip162219.m`
-- no model literals here either.

x0 is TRUE REST here (all-zero, verified: MATLAB checked the tip holds to
0.122 mm std over the whole pre-actuation stretch, well before the window
starts) -- same convention as square/chirp/veronica.

Usage (inside the WSL venv, from examples/simulation/pcs):

    python simulate_isupport_exp_tip162219.py
    python simulate_isupport_exp_tip162219.py --ladder   # rung 0 only, no rollout
...
BAGS = {
    "tip162219": "Eco50_2.5bar_default_tip_162219",
}

# python_exporter_tip162219.m writes
# "Eco50_2.5bar_default_tip_162219_py_tip162219.mat", i.e. tag "_tip162219"
# under the existing {bagname}_py{tag}.mat convention -- no new loading path
# needed.
DEFAULT_TAG = "_tip162219"
...
class ConstantInput(eqx.Module):
    """Constant per-chamber pressure, no filter dynamics (rungs 1-2).

    control_state is held fixed at act_values from t=0 -- used only where the
    MATLAB reference is also unfiltered/at steady state, so there is no
    transient to reproduce.
    """

    act_values: jax.Array  # (nact,) Pa

    def __call__(self, state: SystemState):
        return self.act_values, jnp.zeros_like(state.control_state)


def _settle_rollout(robot, act_values_pa, t1=3.0, rtol=1e-3, atol=1e-4):
    """Integrate from x0=0 to steady state under a constant (unfiltered) input."""
    n_q = robot.num_active_strains
    controller = ConstantInput(act_values=jnp.asarray(act_values_pa))
    initial_state = SystemState(
        t=0.0,
        y=jnp.zeros((2 * n_q,)),
        u=jnp.zeros((robot.num_actuators,)),
        control_state=jnp.asarray(act_values_pa),
    )
    save_ts = jnp.asarray([0.0, t1])
    traj = robot.rollout_closed_loop_to(
    ...
    q_final = jnp.asarray(traj.y[-1, :n_q])
    return q_final


def check_gravity_sag(robot, info, data) -> bool:
    """Ladder rung 1: gravity sag, u = 0, integrated to steady state."""
    q_final = _settle_rollout(robot, jnp.zeros((robot.num_actuators,)))
    p_tip = np.asarray(robot.forward_kinematics(q_final, s=info["s_tip"]))[0:3, 3]
    ref_tip = np.atleast_1d(np.asarray(data["rung1_tip_sag"], dtype=float)).ravel()
    d = float(np.linalg.norm(p_tip - ref_tip))
    print("\n===== LADDER RUNG 1 : gravity sag (u = 0, steady state) =====")
    ...
    print(f"  |diff|    : {d*1e3:.6f} mm")
    ok = d * 1e3 < 0.05
    print(f"VERDICT     : {'PASS' if ok else 'FAIL'} (mass/inertia/E/Poi/gravity direction)")
    print("=" * 55)
    return ok


def check_single_chamber(robot, info, data) -> bool:
    """Ladder rung 2: one chamber at a constant force, steady state, all 6 chambers."""
    chamber_area = float(data["chamber_area"])
    F_const = float(data["rung2_force"])
    ref = np.atleast_2d(np.asarray(data["rung2_tip"], dtype=float))  # (nact, 3), MODEL order
    n_ch = robot.num_actuators
    print("\n===== LADDER RUNG 2 : single-chamber constant force, steady state =====")
    print(f"F = {F_const:.1f} N per chamber (MODEL order, gains = 1, no perm)")
    all_ok = True
    for k in range(n_ch):
        act = np.zeros(n_ch)
        act[k] = F_const / chamber_area  # Pa, so chamber_area * act = F_const N
        q_final = _settle_rollout(robot, jnp.asarray(act))
        p_tip = np.asarray(robot.forward_kinematics(q_final, s=info["s_tip"]))[0:3, 3]
        ref_tip = ref[k]
        d = float(np.linalg.norm(p_tip - ref_tip))
        ok = d * 1e3 < 1.0
        all_ok &= ok
        print(
            f"  chamber {k + 1}: soromox [{p_tip[0]*1e3:9.4f} {p_tip[1]*1e3:9.4f} {p_tip[2]*1e3:9.4f}] mm | "
            f"MATLAB [{ref_tip[0]*1e3:9.4f} {ref_tip[1]*1e3:9.4f} {ref_tip[2]*1e3:9.4f}] mm | "
            f"diff {d*1e3:8.4f} mm  {'OK' if ok else 'FAIL'}"
        )
    print(f"VERDICT     : {'PASS' if all_ok else 'FAIL'} (chamber clock / perm_idx / dc radius / force map)")
    print("=" * 55)
    return bool(all_ok)


def check_step_response(robot, info, data, solver=None) -> bool:
    """Ladder rung 3: single-chamber step with the lag, transient trajectory."""
    chamber_area = float(data["chamber_area"])
    tau = float(data["tau"])
    F_const = float(data["rung3_force"])
    chamber = int(np.asarray(data["rung3_chamber"])) - 1  # 1-based -> 0-based
    t_step = float(data["rung3_t_step"])
    ref_t = np.atleast_1d(np.asarray(data["rung3_time"], dtype=float)).ravel()
    ref_tip = np.atleast_2d(np.asarray(data["rung3_tip"], dtype=float))  # (3, M)
    ...
    initial_state = SystemState(
        ...
        y=jnp.zeros((2 * n_q,)),
        u=jnp.zeros((robot.num_actuators,)),
        control_state=jnp.zeros((n_ch,)),  # u_fh(0) = 0, a genuine step -- see the module docstring
    )
    save_ts = jnp.asarray(ref_t)
    save_dt = float(np.mean(np.diff(ref_t)))
    traj = robot.rollout_closed_loop_to(
        initial_state=initial_state,
        controller=controller,
        t1=float(ref_t[-1]),
    ...
    q_ts = np.asarray(traj.y[:, :n_q])
    fk_tip = jax.jit(partial(robot.forward_kinematics, s=info["s_tip"]))
    p_tip = np.asarray(jax.vmap(fk_tip)(jnp.asarray(q_ts)))[:, 0:3, 3].T  # (3, M)

    diff = p_tip - ref_tip[:, : p_tip.shape[1]]
    rms = float(np.sqrt(np.mean(np.sum(diff**2, axis=0))))
    mx = float(np.max(np.linalg.norm(diff, axis=0)))
    print("\n===== LADDER RUNG 3 : single-chamber step response (2.8 Hz lag) =====")
    print(f"chamber {chamber + 1}, F = {F_const:.1f} N, step at t = {t_step:.3f} s, tau = {tau*1e3:.3f} ms")
    print(f"transient RMS diff : {rms*1e3:.4f} mm | max diff : {mx*1e3:.4f} mm")
    ok = rms * 1e3 < 1.0
    print(f"VERDICT     : {'PASS' if ok else 'FAIL'} (Eta, lag time constant, lag-on-pressure placement)")
    print("=" * 55)
    return ok
...
def run_bag(bag_key: str, args) -> dict:
    ...
    rung1 = check_gravity_sag(robot, info, data)
    rung2 = check_single_chamber(robot, info, data)
    rung3_solver = {"tsit5": Tsit5, "kvaerno5": Kvaerno5}[args.solver]()
    rung3 = check_step_response(robot, info, data, solver=rung3_solver)
    if args.ladder:
        return {"bag": bag_key, "rung0": rung0, "rung1": rung1, "rung2": rung2, "rung3": rung3}
    ...
    result = {
        "bag": bag_key, "rung0": rung0, "rung1": rung1, "rung2": rung2, "rung3": rung3,
        "met_sx": met_sx, "roll": roll, "data": data,
    }
...
def main():
    ...
    ap.add_argument("--bag", choices=["tip162219"], default="tip162219")
