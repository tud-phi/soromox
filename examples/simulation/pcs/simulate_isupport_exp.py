"""AM-I Support: soromox rollout of the MATLAB-identified GVS model.

Port of the SoRoSim/GVS identification pipeline in `am_isupportGVS`. Every
parameter -- geometry, material constants, per-actuator gains, the chamber
clock, the channel permutation, the pressure lag, the fit window -- is read
from the `.mat` exported by `python_exporter.m`. There are deliberately NO
model literals in this file: if the MATLAB side is re-identified, re-export and
re-run, and the two stay in step.

The export also carries the MATLAB reference rollout (`p_tip_sim`, `p_mid_sim`)
on the same time grid, so this script scores parity between the two
SIMULATIONS as well as each simulation against VICON.

Usage (inside the WSL venv, from examples/simulation/pcs):

    python simulate_isupport_exp.py --bag square
    python simulate_isupport_exp.py --bag chirp
    python simulate_isupport_exp.py --bag both --no-show
    python simulate_isupport_exp.py --bag square --ladder   # rung 0 only, no rollout
...
BAGS = {
    "square": "2026-06-08-12-11-25",
    "chirp": "2026-06-08-11-59-18",
}
...
def main():
    ...
    ap.add_argument("--bag", choices=["square", "chirp", "both"], default="square")
    ...
    ap.add_argument(
        "--tag",
        default="",
        help="geometry-variant suffix of the exported .mat and of every output "
        "file (e.g. _g35s178); empty uses the baseline export",
    )
    ...
    bags = ["square", "chirp"] if args.bag == "both" else [args.bag]
    results = [run_bag(b, args) for b in bags]
