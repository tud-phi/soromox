...
if __name__ == "__main__":
    ...
    params = {
        ...
        "L": 0.25 * jnp.ones((num_segments,)),  # default: 1e-1
        "r": 0.025 * jnp.ones((num_segments,)),  # default: 2e-2
        "rho": rho,
        "g": jnp.array([0.0, 0.0, 0.0]),  # Gravity vector [m/s^2]
        "E": 2.5e5 * jnp.ones((num_segments,)),  # Elastic modulus [Pa]
        "G": 0.912e5 * jnp.ones((num_segments,)),  # Shear modulus [Pa]
    }
    params["D"] = 1e-2 * jnp.diag(
        (
            jnp.repeat(
    ...
    active_tendon_routing_params = {
            "ry": jnp.array([0.02, -0.02, 0.0, 0.0]),
            "rz": jnp.array([0.0, 0.0, 0.02, -0.02]),
    ...
    print("Global epsilon =", robot.global_eps)
    ...
    u = jnp.array([2e-1, 0.0, 0.0, 0.0])  # Tendon tensions [N]
    print("u =\n", u)
    ...
    # =====================================================
    # Configuration upon time
    # =====================================================
    plt.figure()
    for i in range(q_ts.shape[1]):
        plt.plot(ts, q_ts[:, i], label=f"q[{i}]")
    plt.xlabel("Time [s]")
    ...
    # evaluate the generalized actuation torques upon time
    def compute_tau_a(q: Array, u: Array) -> Array:
        A = robot.actuation_matrix(q)
        tau_a = A @ u
        return tau_a
    tau_a_ts = jax.vmap(jax.jit(partial(compute_tau_a, u=u)))(q_ts)
    # plot the generalized actuation torques upon time
    plt.figure()
    for i in range(tau_a_ts.shape[1]):
        plt.plot(ts, tau_a_ts[:, i], label=f"Tau_a[{i}]")
    plt.xlabel("Time [s]")
    plt.ylabel("Generalized Actuation Torques")
    plt.title("Generalized Actuation Torques over Time")
    plt.legend()
    plt.grid(True)
    plt.box(True)
    plt.tight_layout()
