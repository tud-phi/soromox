...
from elastica import BaseSystemCollection, Forcing, CallBacks, Constraints, Damping, PositionVerlet, CosseratRod, AnalyticalLinearDamper, CallBackBaseClass, defaultdict, OneEndFixedBC, Contact, RodSelfContact

from muscles import MuscleGroup, ApplyMuscleGroups
from tendon import Tendon1, Tendon2, Tendon3, Tendon4
from elastica import GravityForces
import os
import time
import pandas as pd


class ArmSimulator(BaseSystemCollection, Constraints, Connections, Forcing, CallBacks, Damping, Contact):
    pass
...
class BaseArmEnvElastica():
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.COLLECT_DATA_FOR_PROCESSING = kwargs.get("COLLECT_DATA_FOR_PROCESSING", False)
        ...
        self.plot_dir = kwargs.get("plot_dir", os.path.join(os.getcwd(), "plots", "arm", "video"))
        self.plot_name = os.path.join(self.plot_dir, self.plot_suffix)
        self.target_position = kwargs.get("target_position", None)

        self.sim_duration = kwargs.get("sim_duration", 10) # second

        self.sim_step = int(round(self.sim_duration/self.dt)) # total simulation steps
    ...
    def reset_arm(self):
        ...
        class ArmCallBack(CallBackBaseClass):
            ...
            def make_callback(self, system, time, current_step: int):
                if current_step % self.every == 0:
                    ...
                    self.callback_params["director"].append(system.director_collection[..., -1].copy())
                    self.callback_params["director_all"].append(system.director_collection.copy())
        ...
        def add_tendons(radius_base, arm):
            ...
            rest_tendon_area = shearable_rod_area*(
                tendon_ratio_radius**2
            )
        ...
        self.tendon_post_processing_list = [defaultdict(list) for _ in self.tendon_groups]
    ...
    def reset_finalize(self):
        ...
        self.arm_do_step, self.arm_stages_and_updates = extend_stepper_interface(self.arm_stepper, self.arm_sim)
        self.time_tracker = np.float64(0.0)
    ...
    def simulate(self, action):
        ...
        arm_tip_position = []
        time = []
        print(f"We are here! and begin the simulation")
        ...
        invalid_arm = _isnan_check(self.arm.position_collection) or _isnan_check(self.arm.velocity_collection)
        
        if invalid_arm:
            print(f"BE carefully !!! Something is NaN output!!!")
            obs = -np.inf
        else:
...
file_path = r"Results\end_effector_position_Tendon_driven_PyElastica.xlsx"
