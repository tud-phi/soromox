...
SOURCE_ROOT = Path(__file__).parents[3] / "src" / "soromox" / "systems"


def test_execution_backend_has_one_public_name() -> None:
    """Avoid duplicate family-local aliases for the execution policy type."""

    from soromox.systems import ExecutionBackend, gvs
...
def test_execution_package_has_a_public_import_path() -> None:
    ...
    import soromox.systems.execution as execution

    assert execution.ExecutionBackend is not None
    assert not (SOURCE_ROOT / "_execution").exists()
...
def test_system_models_only_import_the_neutral_execution_api() -> None:
    ...
    assert "soromox.systems.execution.warp" not in model_sources
...
def test_warp_families_only_share_through_common_modules() -> None:
    ...
    assert "soromox.systems.execution.warp.pcs" not in gvs_sources
    assert "soromox.systems.execution.warp.gvs" not in pcs_sources
    assert "soromox.systems.execution.warp.gvs" not in common_sources
    assert "soromox.systems.execution.warp.pcs" not in common_sources
