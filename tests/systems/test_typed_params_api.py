...
def _expected_spatial_material_damping(
    ...
    return blk_diag(vmap(jnp.diag)(length[:, None] * coefficient[:, None] * damping_diag))
...
def test_isupport_inherits_material_damping_path():
    params = ISupportParams(
        ...
        chamber_angle_offset=jnp.array([0.0], dtype=jnp.float64),
    )
    ...
    expected_diag = params.length[0] * params.material_damping_coefficient[0] * jnp.array(
