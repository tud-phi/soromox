...
class PCS(DynamicalSystem):
    ...
    def jacobian_bodyframe_batched(self, q: Array, s_ps: Array) -> Array:
        ...
        J_local_ps = jnp.einsum("ijk, kl->ijl", J_local_ps_, self.B_xi)  # shape (N, 6, num_active_strains)
    ...
    def jacobian_and_derivative_bodyframe_batched(
        ...
        """
        J_local_ps_, Jd_local_ps_ = self._J_Jd_local_batched(q, qd, s_ps)  # shape (N, 6, num_strains)

        J_local_ps = jnp.einsum("ijk, kl->ijl", J_local_ps_, self.B_xi)  # shape (N, 6, num_active_strains)
        Jd_local_ps = jnp.einsum("ijk, kl->ijl", Jd_local_ps_, self.B_xi)  # shape (N, 6, num_active_strains)
